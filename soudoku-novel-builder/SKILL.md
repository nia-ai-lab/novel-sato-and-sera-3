...
6. 最初の制作フェーズでは本文を先に書かない。まず「全体プロット」「主要キャラクター設定」「主要キャラクター画像」「背景イメージ」を揃える。
7. 初期セットが揃ったら、必ず一度ユーザ確認を入れる。承認前に第1章本文へ進まない。
8. ユーザが曖昧なら、最初に次を短く聞く: テーマ、ジャンル、核となる感情、想定ボリューム、避けたい要素、画像トーン。
